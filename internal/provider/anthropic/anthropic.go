...
func (c *client) readStream(ctx context.Context, resp *http.Response, out chan<- provider.Chunk) {
	...
	for scanner.Scan() {
		...
		if err := json.Unmarshal(dataBytes, &ev); err != nil {
			send(provider.Chunk{Type: provider.ChunkError, Err: provider.StreamDecodeError(c.name, string(dataBytes), err)})
			return
		}
		...
		case "content_block_delta":
			...
			case "input_json_delta":
				if tc := tools[ev.Index]; tc != nil && ev.Delta.PartialJSON != "" {
					...
					if bucket := b.Len() / 2048; bucket > argBuckets[ev.Index] {
						argBuckets[ev.Index] = bucket
						if !send(provider.Chunk{Type: provider.ChunkToolCallArgsDelta, ToolCall: &provider.ToolCall{ID: tc.ID, Name: tc.Name}, ArgChars: b.Len()}) {
							return
						}
					}
				}
